...
def compute_metrics(queries, database, llama_feats,sam_feats,metric='cosine',
    ...
    assert retrieval_mode in ["image2recipe", "recipe2image"], "retrieval mode error"
    ...
    dists1 = pairwise_distances(queries, database, metric=metric)
    
    
    #add rerank  distance 
    if  retrieval_mode == "image2recipe":
        dists_rerank_llama  = pairwise_distances(queries,llama_feats,metric=metric)
        dists_rerank_sam  = pairwise_distances(sam_feats,database,metric=metric)

    else:
        dists_rerank_llama  = pairwise_distances(llama_feats,database, metric=metric)
        dists_rerank_sam  = pairwise_distances(queries,sam_feats,metric=metric)
        
    
    

    dists =  dists_rerank_llama * dists_rerank_sam * dists1
